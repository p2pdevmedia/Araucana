...
export function ServicesGrid({ services }: { services: Service[] }) {
  return <div className="services-grid new-services-grid">{services.map((service) => {
    ...
    const video = service.number === "01" ? <video className="service-card-video" autoPlay muted loop playsInline preload="metadata" aria-hidden="true"><source src="/video-aeropuerto.mp4" type="video/mp4" /></video> : null;
